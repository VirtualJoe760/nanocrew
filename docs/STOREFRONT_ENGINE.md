...
**Where the builder's guidance comes from (and a gap).** The forge's headless Claude is steered
entirely by **per-repo** files: `brand.json` (data) + `briefs/01-BRAND.md` & `02-TEST.md` (the
dynamic task) + the template's `TEMPLATE.md`/`VOCABULARY.md` (hard rules + word→block mapping) + a
thin `CLAUDE.md` that just `@`-includes `AGENTS.md`. As of 2026-06-15 there is **no global
`~/.claude/CLAUDE.md` on the droplet** — so workflow-level guidance (you're branding a Nanocrew
storefront; never edit `brand.json`; catalogue/auth/checkout come from the platform — don't
reinvent them; always `pnpm run build` before finishing) lives only in the briefs, repeated per
job. A global forge `CLAUDE.md` carrying those invariants is a planned hardening so every build
holds the same bar regardless of which brief generated it.
