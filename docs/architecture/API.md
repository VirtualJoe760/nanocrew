...
| GET/POST/DELETE | `/api/creator/stores/:slug/collaborators` | bearer | **Owner-only** collaborator management (P4.1). GET lists (owner never listed); POST `{ email }` invites an existing creator (`404` if no account yet, `409` if the owner), idempotent; DELETE `{ collaboratorId }` removes, scoped to the store. Collaborators design + manage via `tenant.ts`, but only the owner administers membership; go-live/publish/domain stay owner-only. |
