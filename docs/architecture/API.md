...
| GET | `/api/creator/site-assets?storeSlug=` | bearer | The CURRENT live site assets ({hero, heroVideo, og, logo, logoKit:{wordmark,mark,appTile,favicon}, sections}) — read by the Design tab's Site-assets dock and Eve's asset flow; pre-kit brands derive the kit read-time from logo_url. POST slots now include `mark` (the square icon master) — assigning `logo`/`mark` re-derives the LogoKit + favicon_url. |
