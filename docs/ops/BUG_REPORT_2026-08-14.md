...
|---|---|---|---|---|
| K1 | High | Account deletion | fixed (pending e2e verify) | `DELETE /api/me` reads `SUPABASE_SERVICE_ROLE_KEY` (unset; env has `SUPABASE_SECRET_KEY`) → Supabase auth identity survives deletion; re-signup with the same email lands half-broken; data-deletion compliance incomplete. Fix: read `SUPABASE_SECRET_KEY` (or set both). |
