...
### B11 · Med · FIXED · Market product tiles open the BRAND sheet, not the product
- Every press handler in [market.tsx](../../src/app/market.tsx) calls `onOpen(item.storeSlug)` — product cards ("New this week" rows with name+price, hero product cards) discard the product id, so tapping "Quiet Horizon · $28.99" lands on Aether Run's landing sheet and the shopper must find the product again. `brand-store.tsx` already renders `ProductDetail`, so the fix is threading an optional productId through onOpen → sheet initial state. **Fixed 2026-08-15:** `products.slug` added to the market API pool + `toItem` ([market+api.ts](../../src/app/api/market+api.ts)); `onOpen(storeSlug, productSlug?)` threaded through Hero/ProductCard → `openStore` → `BrandStore initialProductSlug` ([market.tsx](../../src/app/market.tsx), [brand-store.tsx](../../src/components/brand-store.tsx)). Brand tiles pass no product and still land on the brand.
...
### B15 · High · FIXED · Product picker header renders under the status bar (16e) — Close unreachable, user trapped
- The full-screen picker modal has no top safe-area inset on iPhone 16e: breadcrumb + "Close" draw behind the clock/battery. Close's touch target is occluded; swipe-dismiss doesn't apply (full-screen modal). Only escape = selecting a product. **Root cause:** a RN `<Modal>` is its own native window, so the app's safe-area context never reaches it and `SafeAreaView` resolves EVERY edge to 0 — the same trap [brand-store.tsx](../../src/components/brand-store.tsx) already documents. **Fixed 2026-08-15:** nested `<SafeAreaProvider>` inside the Modal (the [site-preview.tsx](../../src/components/site-preview.tsx) idiom) in [ProductPicker.tsx](../../src/components/designer/ProductPicker.tsx) + [ProductDetailSheet.tsx](../../src/components/designer/ProductDetailSheet.tsx). **Verified on the 16e sim**: header now clears the status bar and Close dismisses. See B18 for the remaining modals.

### B16 · Low · FIXED · Mic permission copy still says "Venus"
- app.json NSMicrophoneUsageDescription: "…talk to Venus, your AI brand consultant." The Eve rename missed the iOS permission strings (needs a native rebuild to take effect). **Fixed 2026-08-15** in both `app.json` and the committed `ios/NanoCrew/Info.plist` (prebuild output is checked in, so both had to change). Ships with the next NATIVE build (52) — not OTA-deliverable.

### B17 · Low · FIXED · Publish success screen: missing icon glyph + rough layout
- "Live on Printful" renders with a "[?]" placeholder glyph and awkward wrapping; Done button crowded. **Cause:** the brand face (Geist) carries symbol glyphs (✦/✓ render fine elsewhere in the same file) but NO COLOR EMOJI, so the 🎉 fell through to a missing-glyph box. **Fixed 2026-08-15** ([FinalizeSheet.tsx](../../src/components/designer/FinalizeSheet.tsx)): 🎉 → ✓ (the glyph the shots hint already uses), centred title/subtitle, and the Done pill stretched full-width instead of shrink-wrapping the label.

### B18 · Med · Safe-area context is dead inside EVERY other modal (audit)
- Same root cause as B15, found by sweeping all modals: **no** modal compensates manually, so any `SafeAreaView edges={['top']}` inside one is running a 0 inset. Correct today: [brand-store.tsx](../../src/components/brand-store.tsx) + [product-detail.tsx](../../src/components/product-detail.tsx) (read the inset from the hook OUTSIDE the modal) and [site-preview.tsx](../../src/components/site-preview.tsx) (nested provider). Fixed in this pass: ProductPicker, ProductDetailSheet. **Still unpatched** (all `transparent` sheets, so content tucks under the island rather than trapping the user): paywall, go-live-composer, site-editor, purchases (×2), returns-inbox, earnings-cockpit, studio-composer, scene-short-composer. The wrapper fix is mechanical, but each needs a visual pass to confirm nothing double-pads — deliberately NOT blind-patched before a tester build.
