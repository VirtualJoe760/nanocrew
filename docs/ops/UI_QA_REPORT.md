...
1. **Systemic: `.json()` called without an `res.ok` check** in ~12 places → on a 5xx the error body is parsed as success (silent failure) or `.json()` throws on HTML (e.g. a 502). Add a shared `fetchJson()` helper that checks `ok` + parses safely, and apply it to: `studio-composer` loadInsights/loadStores/mutatePost/refundOrder, `studio-dashboard` load, `studio.tsx` createStore, `design.tsx` runGenerate/doCombine, `account.tsx` /api/me + openPayouts, `brand-store.tsx`, `earnings-cockpit.tsx`, `site-editor.tsx` load.
