...
proven clothing-store templates in advance, structured around exactly the data Eve
...
> SSH-from-server broke on the managed/serverless host (the Cloud Run backend can't hold a
...
| Concern | `src/lib/provision.ts` (app server, Cloud Run) | `forge-worker/worker.mjs` (droplet, systemd) |
|---|---|---|
...
- `TEMPLATE.md` — the block spec + hard rules (the forge composes from this); `VOCABULARY.md` —
  the creator-words → block dictionary, read by **Eve** when authoring the brief (not the forge)
...
> ⚠️ **The only place to get this right is the app server (Cloud Run).** `brand.json.apiBase`,
...
> unset on Cloud Run, every newly provisioned brand ships with an empty `apiBase` → placeholder
...
1. **`01-BRAND.md`** (`authorBrandBrief` — AI-authored by Eve via `gemini-2.5-pro`; falls back to
   the `buildBrandBriefFallback` mail-merge) — a concrete, block-by-block build plan. **Eve does the
...
creator told Eve — same constraints, "apply ONLY the requested change" framing.
...
decoding the creator (Eve already did that)** — plus the **per-repo** files: `brand.json` (data),
...
the template repo but is read by **Eve** (when authoring the brief), not the forge. See
[../studio/FORGE_AI.md](../studio/FORGE_AI.md).

## Revisions — the same queue, a working branch

When a creator asks Eve to edit a live site, the change rides the **same `store_revisions`
