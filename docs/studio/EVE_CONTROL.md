...
| **BRANDS** (label; id `site`) | `onShowBrands()` | summons the **Your-Brands deck** — pick the brand, then its **Console** (Edit site · Posts · Sell · Settings) via the deck's edit action. Voice edits stay reachable by asking her (`edit-site` intent → `EveDeveloping`). (2026-08-17; was `onGo({state:'developing'})`, which stranded the console once the wheel replaced the deck as the main path) |
