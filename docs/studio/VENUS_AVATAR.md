...
- **`src/components/backgrounds/venus-head-scene.tsx`** (the live POC, rendered by the Lab) — now
  ...
  - **Hair — a short Mia-Wallace BOB** (user direction: mature, not "little girl"): `Wolf3D_Hair`
    kept **skinned + visible** as a soft translucent cool **volume** (`MeshBasicMaterial`, FrontSide,
    depthWrite) + a glowing `EdgesGeometry` **rim**, then **clipped to chin length** by a world-space
    `THREE.Plane` (`gl.localClippingEnabled`) at `aimY + HAIR_BOB_Y_OFFSET` — applied to BOTH the
    solid and the rim so the cut is consistent; keeps the fringe, cuts the length. (Earlier
    long-hair / sparse-points / edge-contour treatments looked straggly or juvenile — replaced.)
  - **Eyes — a readable IRIS:** a sprite with a generated iris texture (dark pupil + bright limbal
    ...
    `LeftEye`/`RightEye` bones so the gaze **tracks the saccades**. (Additive: the alpha-0 pupil
    reads dark, so the iris is visible — a plain glow-blob was not.)
