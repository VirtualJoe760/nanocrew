...
{"intent":"create-brand"|"edit-site"|"new-design"|"site-asset"|"write-post"|"digest"|"done"|"none","storeSlug":"...","idea":"...","slot":"hero"|"logo"|"mark"|"og","topic":"...","ask":"..."}
...
- "site-asset": they ask for a graphic FOR THEIR WEBSITE — a hero/banner image, a logo, or a social/share card ("new hero for my site", "make me a logo", "redo the banner"). idea = the graphic concept, short (omit if they gave none). slot = "hero" for hero/banner/header imagery, "logo" for the wide wordmark logo, "mark" for the square icon / app icon / favicon, "og" for the social/share/preview card; omit slot when they didn't say which. A design for a PRODUCT (tee, hoodie, print) is "new-design", not this.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const out = {
      ...
      ...(intent === 'site-asset' && ['hero', 'logo', 'mark', 'og'].includes((parsed as { slot?: string }).slot ?? '')
